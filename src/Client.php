...
class Client
{
    ...
    public const ENV_PARTNER_ID_NAME = '846156';

    public const ENV_SHOP_ID_NAME = '58852385';
    ...
    public function __construct(array $config = [])
    {
        ...
        $this->nodes['apiAuthentication'] = new Nodes\ApiAuthentication\ApiAuthentication($this);
    }
    ...
    public function requestShopAuthorization($uri, array $headers = [], $data = [])
    {   
        $redirect_url = 'https://google.com';
        $timest = time();
        $base_string = $this->partnerId.$uri.$timest;
        $sign = hash_hmac('sha256', $base_string, $this->secret);
        $url = $this->baseUrl.$uri.'?partner_id='.$this->partnerId.'&timestamp='.$timest.'&sign='.$sign.'&redirect='.$redirect_url;
        return $url;
    }
}
